...
static inline int fls(int x){ return x ? 32 - clz32(x) : 0; }
#include "hw/misc/mxs_bchlib.h"

/* upstream GPL libbch.c uses VLAs by design */
...
static inline const char *gf_poly_str(const struct gf_poly *p)
{
    static char buf[256];
    unsigned i;
    int n = 0;

    for (i = 0; i <= p->deg && n < 250; i++) {
        n += snprintf(buf + n, sizeof(buf) - n, "%u ", p->c[i]);
    }
    return buf;
}
...
void free_bch(struct bch_control *bch)
{
	...
	if (bch) {
		g_free(bch->a_pow_tab);
		g_free(bch->a_log_tab);
		g_free(bch->mod8_tab);
		g_free(bch->ecc_buf);
		g_free(bch->ecc_buf2);
		g_free(bch->xi_tab);
